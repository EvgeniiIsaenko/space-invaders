...
//Populate shields vector with shields
void Game::initShields() {
	//Clear the existing shields vector and deallocate memory
	for (auto& shield : shields) {
		delete shield;
	}
	shields.clear();

	//Populate new shields vector 
	int shieldSpacing = 167;
	int shieldYOffset = 600;
	for (int i = 0; i < 6; i++) {
		shields.push_back(new Shield(i * shieldSpacing, shieldYOffset));
	}
}

//Initialize sound buffers and sounds
void Game::initSounds() {
	invaderHitSoundBuffer.loadFromFile("Sounds/invaderHit.mp3");
	invaderHitSound.setBuffer(invaderHitSoundBuffer);
	playerHitSoundBuffer.loadFromFile("Sounds/playerHit.mp3");
...
void Game::checkInvaderCollisions() {
		...
		for (int j = 0; j < invaders[i].size(); j++) {
			...
			for (int k = 0; k < playerBullets.size(); k++) {
				if (playerBullets[k]->getBullet().getGlobalBounds().intersects(invaderHitbox.getBounds())) {
					...
					addExplosion(playerBullets[k]->getXPosition(), playerBullets[k]->getYPosition());
					//Play invader hit sound
...
void Game::checkShieldCollisions() {
	...
	for (int i = 0; i < shields.size(); i++) {
		sf::VertexArray shieldHitbox = shields[i]->getVertices();
		// See if invader bullets are hitting the shield, if they are, damage it
		for (int j = 0; j < invaderBullets.size(); j++) {
			if (invaderBullets[j]->getBullet().getGlobalBounds().intersects(shieldHitbox.getBounds())) {
				//Add an explosion where the bullet hit
				addExplosion(invaderBullets[j]->getXPosition(), invaderBullets[j]->getYPosition());
				delete invaderBullets[j];
				invaderBullets.erase(invaderBullets.begin() + j);
				shields[i]->TakeDamage();
		...
		//See if player bullets are hitting the shield, if they are, the player bullet should be destroyed, but we don't want
		//to damage the shield
		for (int k = 0; k < playerBullets.size(); k++) {
			if (playerBullets[k]->getBullet().getGlobalBounds().intersects(shieldHitbox.getBounds())) {
				//Add an explosion where the bullet hit
				addExplosion(playerBullets[k]->getXPosition(), playerBullets[k]->getYPosition());
				delete playerBullets[k];
				playerBullets.erase(playerBullets.begin() + k);
...
void Game::checkPlayerCollisions() {
	...
	for (int i = 0; i < invaderBullets.size(); i++) {
		if (invaderBullets[i]->getBullet().getGlobalBounds().intersects(playerHitbox.getBounds())) {
			//Add an explosion where the bullet hit
			addExplosion(invaderBullets[i]->getXPosition(), invaderBullets[i]->getYPosition());
...
void Game::updateInvaders() {
	//If all invaders have been killed, spawn more and respawn shields
